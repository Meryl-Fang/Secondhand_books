...
def get_book_series_info(search_url,total_page_number):

	
	urls = [search_url+ str(i+1) for i in range(total_page_number)]
	result = []
	...
	for i in urls:
		...
		for tags in soup.find_all('div',id='listBox',class_='result-list'):#,class_='subject-item'):
	
			info_list=[]

			for i in tags.find_all('div',class_='item clearfix'):
				title = i.get('bookname')
			
				print(title)


			#pub_tag = tags.find('div',class_='zl-isbn-info').get_text().strip()
			#print(title)
...
def save(data):
	file_csv = open('kfz_test.csv','w+',newline='')
...
if __name__ == '__main__':
	url='http://search.kongfz.com/item_result/?select=0&key=E.B.%E6%80%80%E7%89%B9&pagenum='
	#login_douban()
	get_book_series_info(url,2)